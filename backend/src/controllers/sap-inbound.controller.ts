...
async function genericInboundHandler(
  ...
  try {
    ...
    logger.error({ err, source }, "sap-inbound: process fail");
...
export async function getInboundEvents(
  ...
  try {
    const list = await listInboundEvents({ source: req.query.source });
...
export async function getInboundEventDetail(
  ...
  try {
    const ev = await getInboundEventById(req.params.id);
...
export async function getTokens(_req: FastifyRequest, reply: FastifyReply) {
  try {
...
export async function delToken(
  ...
  try {
    const ok = await deleteToken(req.params.id);
