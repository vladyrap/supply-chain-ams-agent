...
export async function getScopeItems(
  ...
  try {
    const items = await listScopeItems({ module: req.query.module as never });
...
export async function getScopeItem(
  ...
  try {
    const item = await getScopeItemByCode(req.params.code);
...
export async function postSuggestForTicket(
  ...
  try {
    const items = await findScopeItemsForTicket(req.body || {});
